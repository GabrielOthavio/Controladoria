...
# O script agora tenta determinar automaticamente o diretório do projeto
# Prioridade: argumento $1 -> variável de ambiente PROJECT_DIR_OVERRIDE -> diretório do próprio script -> fallback antigo
if [ -n "$1" ]; then
    PROJECT_DIR="$1"
...
if [ ! -d "$PROJECT_DIR" ]; then
    # fallback para local esperado antigo (compatibilidade)
...
print_info "Usando diretório do projeto: $PROJECT_DIR"
cd "$PROJECT_DIR"
